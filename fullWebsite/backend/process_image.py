...
def process_image(img_path):
    ...
    with open('D:/Projects/gfg-hackthon/fullWebsite/backend/leaf_or_not_svm', 'rb') as f:
    ...
    if(isleaf == -1):
        ...
        model = load_model('D:/Projects/gfg-hackthon/fullWebsite/backend/Model_v1_resnet50_4epoch')
        img_tensor = np.expand_dims(img_cnn, axis=0)
        fast_pred = model(img_tensor, training=False)
        key = np.argmax(fast_pred)
        print(d[key])
        with open('D:/Projects/gfg-hackthon/fullWebsite/backend/Result.json', 'r') as f:
            label_mapping = json.load(f)
        f.close()
        
        return (d[key], label_mapping[d[key]])
